      ...
      <div className={`bg-[#061a33] text-slate-200 text-xs transition-all duration-300 ${isScrolled ? 'py-1 hidden md:block' : 'py-2'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-wrap justify-between items-center gap-2">
          ...
          <div className="flex items-center space-x-3 text-xs">
            ...
            {/* NLP Login Button (Matching Poster Top Right) */}
            <a
              href={SCHOOL_INFO.erpUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center space-x-1.5 border border-emerald-400 text-emerald-300 hover:bg-emerald-500 hover:text-white font-extrabold px-3 py-1 rounded-lg text-[11px] transition-all shadow-sm"
            ...
            <button
              ...
              id="parent-portal-header-btn"
              className="inline-flex items-center space-x-1 bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-500 hover:to-amber-600 text-navy-950 font-black px-2.5 py-1 rounded-lg text-[11px] shadow-sm transition-all transform hover:scale-105"
              title="Parent & Student Portal"
            >
              <Users className="w-3 h-3 text-navy-950" />
              <span>Parent Portal</span>
            </button>

            {/* Admin CRM toggle button */}
            <button
              onClick={onOpenAdminCRM}
              id="admin-cms-btn"
              className="text-slate-400 hover:text-white px-2 py-1 rounded border border-slate-700 hover:border-slate-500 text-[11px] transition-colors"
              title="School Administration & Admissions CRM"
            >
              Admin CMS
            </button>
          </div>
